...
require_relative 'spell/bonuses/lores/arcane_lore_bonuses'
require_relative 'spell/bonuses/lores/bard_base_lore_bonuses'
require_relative 'spell/bonuses/lores/cleric_base_lore_bonuses'
require_relative 'spell/bonuses/lores/empathic_lore_bonuses'
require_relative 'spell/bonuses/lores/major_elemental_lore_bonuses'
require_relative 'spell/bonuses/lores/major_mental_lore_bonuses'
require_relative 'spell/bonuses/lores/major_spiritual_lore_bonuses'
require_relative 'spell/bonuses/lores/minor_elemental_lore_bonuses'
require_relative 'spell/bonuses/lores/minor_mental_lore_bonuses'
require_relative 'spell/bonuses/lores/minor_spiritual_lore_bonuses'
require_relative 'spell/bonuses/lores/paladin_lore_bonuses'
require_relative 'spell/bonuses/lores/ranger_base_lore_bonuses'
require_relative 'spell/bonuses/lores/savant_base_lore_bonuses'
require_relative 'spell/bonuses/lores/sorcerer_base_lore_bonuses'
require_relative 'spell/bonuses/lores/wizard_base_lore_bonuses'
  ...
  module Gemstone
    module Spell
      module Bonuses
        module Lore
          ##
          # LoreBonusFormatter provides utility methods to access structured
          # spell lore bonus data stored in `SpellLoreBonuses.table`.
          #
          ...
          #
          class LoreBonusFormatter
            ##
            # Fetches a specific effect from the lore bonus table.
            #
          ...
          #
          # Can optionally use the `:lore_used` proc to auto-calculate ranks.
          #
          # Returns a Numeric value (with max cap applied) or a hash of such values.
          #
          class LoreBonusEvaluator
            ##
            ...
            # @param [Integer] ranks The number of lore ranks trained (default: from :lore_used if available)
            # @return [Numeric, nil] The total bonus amount, or nil if invalid
            #
            # @example
            #   effect = LoreBonusFormatter.effect_for(:minor_spiritual, 104, "increased disesase resistance")
            #   LoreBonusEvaluator.evaluate_bonus(effect, 42)
            def self.evaluate_bonus(effect, ranks = nil)
              return nil unless effect.is_a?(Hash)

              # Determine ranks from lore reference if not passed
              ranks ||= if effect[:lore_used].respond_to?(:call)
                          effect[:lore_used].call
                        else
                          0
                        end
